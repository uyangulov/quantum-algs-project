...
class QuantumCircuit:
    ...
    Attributes:
        ...
        num_qubits (int): The number of qubits in the quantum circuit. Dynamically updated as gates are added.
    ...
    Methods:
        ...
        concat(other_circuit: 'QuantumCircuit'):
            Appends another quantum circuit to the end of this quantum circuit.

        from_qiskit(qiskit_circuit: QiskitQuantumCircuit):
    ...
    def __init__(self, gates: list[Gate] = None):
        ...
        Args:
            gates (list[Gate], optional): A list of Gate objects. Defaults to None.
        """
        self.gates = gates if gates is not None else []
        self.num_qubits = (
            max((max(gate.qubit_indices) for gate in self.gates), default=0) + 1
            if self.gates
            else 0
        )
    ...
    def append(self, gate: Gate):
        ...
        self.num_qubits = max(self.num_qubits, max(gate.qubit_indices) + 1)
    ...
    def compression_list(self):
        ...
        layer_to_gate_indices = []

        for i, gate in enumerate(self.gates):
            qubit_indices = gate.qubit_indices
            maximum_layer = max(
                [qubit_to_current_layer[qubit_id] for qubit_id in qubit_indices]
            )

            if maximum_layer >= len(layer_to_gate_indices):
                layer_to_gate_indices.append([])

            layer_to_gate_indices[maximum_layer].append(i)

            for qubit_id in qubit_indices:
                qubit_to_current_layer[qubit_id] = maximum_layer + 1

        return layer_to_gate_indices
    ...
    def concat(self, other_circuit: 'QuantumCircuit'):
        ...
        Args:
            other_circuit (QuantumCircuit): The quantum circuit to append.
        """
